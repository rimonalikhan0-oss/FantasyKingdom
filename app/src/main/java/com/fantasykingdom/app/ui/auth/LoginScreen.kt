...
/** Feature #2 (part 1) — Login screen backed by Firebase Authentication. */
@Composable
fun LoginScreen(onLoginSuccess: () -> Unit, onNavigateToSignUp: () -> Unit) {
    ...
    Column(
        ...
        androidx.compose.material3.Icon(
            ...
            modifier = Modifier.height(56.dp).padding(bottom = 12.dp)
        )
        ...
        Row(modifier = Modifier.fillMaxWidth().padding(top = 6.dp), horizontalArrangement = Arrangement.End) {
            TextButton(onClick = { /* Password reset flow can call viewModel via a dedicated dialog */ }) {
                Text("Forgot Password?")
            }
        }

        Spacer(Modifier.height(12.dp))
        FKPrimaryButton(text = "Login", isLoading = uiState.isLoading) {
        ...
        Row(
            ...
            TextButton(onClick = onNavigateToSignUp) { Text("Sign Up", fontWeight = FontWeight.SemiBold) }
        }
    }
}
